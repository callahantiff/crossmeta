...
load_agil <- function (gse_names, data_dir, gpl_dir, ensql) {
    ...
    for(i in 1:length(esets)){
      if (length(esets) > 0) {
        ...
        gse_meta[i, "channels"] = ifelse((any(grepl('ch2', pdat_names))), 'two', 'one')
        gse_meta[i, "data_type"] = ifelse(length(grep(".*txt*", 
...
load_agil_plat <- function (eset, ch, gse_dir, gse_name, ensql) {
    ...
    if (ch == 'two') {
      # load non-normalized txt files and normalize
      # 
      # elist=limma::read.maimages(elist_paths, source = "agilent",
      #                      columns=list(Rf="rMeanSignal", Gf="gMeanSignal", Rb="rProcessedSignal", Gb="gProcessedSignal"))
      ...
      # transfer to eset - assuming all of the two-color microarray experiments used a common reference
